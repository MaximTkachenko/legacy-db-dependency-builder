    ...
    public class Searcher
    {
        ...
        public Searcher(SearchConfig config)
        {
            ...
            foreach (var root in config.DbPath)
            {
                ...
                foreach (RefObjectType t in Enum.GetValues(typeof(RefObjectType)))
                {
                    ...
                    _sql[root.Key][t] = Directory.GetFiles(folder, "*.sql").Select(x => (Path.GetFileNameWithoutExtension(x), File.ReadAllText(x))).ToList();
                }
            }
            ...
            {
                _csharp[Path.GetFileNameWithoutExtension(sln)] = Directory.GetFiles(Path.GetDirectoryName(sln), "*.*", SearchOption.AllDirectories)
        ...
        public List<RefObject> Find(RefObject obj)
        {
            ...
            foreach (var db in _sql)
            {
                ...
                foreach (var t in rules)
                {
                    ...
                    result.AddRange(scripts
                        .Where(x => !(x.Name.Equals(obj.Name, StringComparison.OrdinalIgnoreCase) && t == obj.Type && db.Key.Equals(obj.Db)))
            ...
            if (obj.Type != RefObjectType.Cs || obj.Type != RefObjectType.Etl)
            {
                ...
                {
                    result.AddRange(sln.Value.Where(x => x.Value.Contains(csharpMapRegex, StringComparison.InvariantCultureIgnoreCase)
